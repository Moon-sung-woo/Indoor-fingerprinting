...
floor_batch_size = 1
batch_size = 1000
learning_rate = 0.0019212068860854148
output_floor = 1
floor_size = int(floor_y_data.max()) + 1#5
loc_size = 2 #x,y
encoder_hidden_num1 = 350
encoder_hidden_num2 = 260
drop_out_param = 0.6239703954508626
EPOCH = 10029
load_model = True
random_num = int(x_4_loc_test.shape[0] * 0.1)
minimum_loss = 999999
minimum_snapshot = []
'''
#TUT 데이터 사용
...
def train_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        idx = random.randint(seq_length, x_4_loc_train.shape[0]) - 1
...
def test_data_random_sample(batch_size = 32):
    ...
    test_y_data = []
    for i in range(batch_size):
        idx = random.randint(seq_length, x_4_loc_test.shape[0]) - 1
        test_x_data.extend(  x_4_loc_test[idx:idx+1]  )
        test_y_data.append( y_4_loc_test[idx] )
...
print()
'''
with tf.variable_scope('filter_4'):
    # ---------------------------------------------------------------------------------------
    X = tf.placeholder(tf.float32, [None, x_4_loc_train.shape[1]])
    in_filter = tf.placeholder(tf.float32, [None, x_4_loc_train.shape[1]])
    ...
    layer1 = tf.contrib.layers.fully_connected(filter1, int(3479.359032132761 ), activation_fn=tf.nn.elu)
    ...
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(2134.079474849024 ), activation_fn=tf.nn.elu)
    ...
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(4369.767198978552 ), activation_fn=tf.nn.elu)
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./filter_4')
        saver.restore(sess, ckpt.model_checkpoint_path)

        for i in range(1000):

            verification = x_4_loc_test.shape[0]
            filter_size = verification * seq_length

            test_x, test_y = test_data_random_sample(x_4_loc_test.shape[0])
        ...
        with open("filter_full_loc_4_best.txt", "w") as f:
        ...
        mean_loss = np.mean(loss)
        print(np.max(loss), np.min(loss), mean_loss, np.std(loss))
        saver.save(sess, './filter_4/loc_floor4_test_result.ckpt')