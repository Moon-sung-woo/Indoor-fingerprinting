...
floor_batch_size = 1
batch_size = 1
learning_rate = 0.0000013105379156717462
output_floor = 1
floor_size = int(floor_y_data.max()) + 1#5
loc_size = 2 #x,y
encoder_hidden_num1 = 350
encoder_hidden_num2 = 260
drop_out_param = 0.7736441708812634
EPOCH = 6978000
load_model = True
random_num = int(x_3_loc_test.shape[0] * 0.1)
minimum_loss = 999999
minimum_snapshot = []
'''
#TUT 데이터 사용
...
def train_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        idx = random.randint(seq_length, x_3_loc_train.shape[0]) - 1
...
def test_data_random_sample(batch_size = 32):
    ...
    test_y_data = []
    for i in range(batch_size):
        idx = random.randint(seq_length, x_3_loc_test.shape[0]) - 1
        test_x_data.extend(  x_3_loc_test[idx:idx+1]  )
        test_y_data.append( y_3_loc_test[idx] )
...
print()
'''
with tf.variable_scope('filter_3'):
    # ---------------------------------------------------------------------------------------
    X = tf.placeholder(tf.float32, [None, x_3_loc_train.shape[1]])
    in_filter = tf.placeholder(tf.float32, [None, x_3_loc_train.shape[1]])
    ...
    layer1 = tf.contrib.layers.fully_connected(filter1, int(4403.78674995623), activation_fn=tf.nn.elu)
    ...
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(3944.832325520039), activation_fn=tf.nn.elu)
    ...
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(1951.6447056980805), activation_fn=tf.nn.elu)
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./test_filter_3')
        saver.restore(sess, ckpt.model_checkpoint_path)

        for i in range(10000):

            verification = x_3_loc_test.shape[0]
            filter_size = verification * seq_length

            test_x, test_y = test_data_random_sample(x_3_loc_test.shape[0])
            test_filter = [np.where(x == 0, 0, 1) for x in test_x]
            loss = sess.run([result_loss], feed_dict={X: test_x, Y: test_y, in_batch_size: verification, in_drop_out: 1.0,
                                                      in_filter: test_filter, in_filter_size: filter_size})
        ...
        with open("filter_full_loc_3_best.txt", "w") as f:
        ...
        test_x, test_y = test_data_random_sample(verification)
        test_filter = [np.where(x == 0, 0, 1) for x in test_x]
        ...
        mean_loss = np.mean(loss)
        print(np.max(loss), np.min(loss), mean_loss, np.std(loss))
        saver.save(sess, './test_filter_3/loc_floor3_test_result.ckpt')