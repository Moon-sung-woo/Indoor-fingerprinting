...
def train_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        idx = random.randint(seq_length, x_1_loc_train.shape[0]) - 1
        train_x_data.extend(x_1_loc_train[idx-seq_length+1: idx+1])
        train_y_data.append( y_1_loc_train[idx] )
    return (train_x_data, train_y_data)
def test_data_sample(x_test,y_test):
    test_x_data = []
    test_y_data = []
...
def test_data_random_sample(batch_size = 32):
    ...
    test_y_data = []
    for i in range(batch_size):
        idx = random.randint(seq_length, x_1_loc_test.shape[0]) - 1
        test_x_data.extend(  x_1_loc_test[idx-seq_length+1 : idx+1]  )
...
print()
'''
with tf.variable_scope('floor1_loc'):
    # ---------------------------------------------------------------------------------------
    ...
    layer1 = tf.contrib.layers.fully_connected(X, int(3.15928963e+03), activation_fn=tf.nn.elu)
    ...
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(3.23335499e+03), activation_fn=tf.nn.elu)
    ...
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(1.71414458e+03), activation_fn=tf.nn.elu)
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./floor1_loc')
        ...
        with open("full_loc_1_best.txt", "w") as f:
    ...
    # 검증
        test_x, test_y = test_data_sample(x_1_loc_test, y_1_loc_test)
        loss = sess.run([result_loss], feed_dict={X: test_x, Y: test_y, in_batch_size: batch_size, in_drop_out: 1.0})
        print( np.max(loss), np.min(loss), np.mean(loss), np.std(loss))
        saver.save(sess, './floor1_loc/loc_floor1_test_result.ckpt')