...
def train_validation(arg):
    ...
    with tf.Session() as sess:
        ...
        for i in range(EPOCH+1):
            ...
            tr_output, step_cost, _  = sess.run([hypothesis, cost, optimizer], feed_dict={X: train_x, Y: train_y, in_batch_size: batch_size, in_drop_out: drop_out_param})
            if(i%200==0):
                print("step : ", i, "cost : ", step_cost, 'output : ', tr_output, 'Y_답:', train_y)

        #검증
        test_x, test_y = test_data_random_sample(x_data.shape[0])
        reult_soft ,y_predict = sess.run([hypothesis, predict], feed_dict={X: test_x, in_batch_size: batch_size, in_drop_out: 1.0})
        cnt = 0
