...
def test_data_random_sample(batch_size = 32, seq_length=1):
    ...
    test_y_data = []
    for i in range(batch_size):
        idx = random.randint(1, x_2_loc_test.shape[0]) - 1
        for j in range(seq_length):
...
with tf.variable_scope('LSTM_2_loc'):
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./LSTM_2_loc')
        ...
        with open("full_LSTM_loc_2_best.txt", "w") as f:
    ...
    #검증
        ...
        mean_loss = np.mean(loss)
        print(np.max(loss), np.min(loss), mean_loss, np.std(loss))
        saver.save(sess, './LSTM_2_loc/LSTM_2_loc_test_result.ckpt')
