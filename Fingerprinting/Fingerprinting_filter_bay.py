...
def train_validation(arg):
    ...
    layer1 = tf.contrib.layers.fully_connected(filter1, int(hidden_leyers[0]), activation_fn=tf.nn.elu)
    # layer1 = tf.contrib.layers.fully_connected(X, int(hidden_leyers[0]), activation_fn=tf.nn.elu)
    drop_1 = tf.nn.dropout(layer1, in_drop_out)
    layer2 = tf.contrib.layers.fully_connected(drop_1, int( hidden_leyers[1]), activation_fn=tf.nn.elu)
    # layer2 = tf.contrib.layers.fully_connected(drop_1, int(hidden_leyers[1]), activation_fn=tf.nn.elu)
...
for floor_step in range(len(train_x_datas)):
    ...
    minimum_snapshot = []
    print('옵티마이즈 세팅')
    myBopt = BayesianOptimization(f=train_validation, domain=domain, initial_design_numdata=7)
    print('옵티마이즈 시작')
    ...
    with open('filter_optimize_result_%s_floor.txt' % floor_step, 'w') as f:
        for param in myBopt.x_opt:
            f.write(str(param)+" ")
    with open('filter_min_optimize_result_%s_floor.txt' % floor_step, 'w') as f:
