...
print()
'''
with tf.variable_scope('floor_loc'):
    # ---------------------------------------------------------------------------------------
    ...
    layer1 = tf.contrib.layers.fully_connected(X, int(1.88472595e+03), activation_fn=tf.nn.elu)
    ...
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(2.36825686e+03), activation_fn=tf.nn.elu)
    ...
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(2.41870067e+03), activation_fn=tf.nn.elu)
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./floor_loc')
        ...
        with open("full_loc_0_best.txt", "w") as f:
    ...
    # 검증
        ...
        saver.save(sess, './floor_loc/loc_floor0_test_result.ckpt')