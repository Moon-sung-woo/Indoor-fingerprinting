...
domain = [{'name': 'lr_rate',
          ...
          {'name': 'hidden size',
           'type': 'continuous',
           'domain': (int(x_data.shape[1]*1.5), int(x_data.shape[1]*3.3)),
           'dimensionality': 3},
          {'name': 'batch size',
           'type': 'discrete',
...
def train_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        ...
        train_y_data.append( loc_y_data[idx] )
...
def test_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        ...
        test_y_data.append( loc_y_test[idx] )
...
def train_validation(arg):
    ...
    #layer1 = tf.contrib.layers.fully_connected(X, int(X.shape[1]*3), activation_fn=tf.nn.elu)
    layer1 = tf.contrib.layers.fully_connected(X, int(2.55664612e+03), activation_fn=tf.nn.elu)
    drop_1 = tf.nn.dropout(layer1, in_drop_out)
    #layer2 = tf.contrib.layers.fully_connected(drop_1, int(X.shape[1]*5), activation_fn=tf.nn.elu)
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(1.87564506e+03), activation_fn=tf.nn.elu)
    drop_2 = tf.nn.dropout(layer2, in_drop_out)
    #layer3 = tf.contrib.layers.fully_connected(drop_1, int(X.shape[1]*8), activation_fn=tf.nn.elu)
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(1.91302401e+03), activation_fn=tf.nn.elu)
    ...
    with tf.Session() as sess:
        ...
        #검증
        test_x, test_y = test_data_random_sample(x_data.shape[0])
