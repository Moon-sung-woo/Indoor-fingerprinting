...
with tf.variable_scope('No_bias_0'):
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./No_bias_0')
        saver.restore(sess, ckpt.model_checkpoint_path)
        for i in range(1000):

            verification = x_0_loc_test.shape[0]
            filter_size = random_num * seq_length

            test_x, test_y = test_data_random_sample(random_num, seq_length)
            test_filter = [np.where(x == 0, 0, 1) for x in test_x]
            loss = sess.run([result_loss], feed_dict={X: test_x, Y: test_y, in_batch_size: random_num,
                                                      in_drop_out: 1.0})
        ...
        with open("NONE_bias_LSTM_loc_0_best.txt", "w") as f:
    ...
    #학습
        ...
        saver.save(sess, './No_bias_0/No_bias_LSTM_0_loc_test_result.ckpt')
    #검증
