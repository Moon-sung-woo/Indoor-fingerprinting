...
def test_data_random_sample(batch_size = 32, seq_length=1):
    ...
    test_y_data = []
    for i in range(batch_size):
        idx = random.randint(1, x_4_loc_test.shape[0]) - 1
        for j in range(seq_length):
...
'''
def rnadom_test(random_num = x_4_loc_test.shape[0]*0.5):
    test_x_data = []
    test_y_data = []
    for i in range(random_num):
        test_x_data = random.sample(range(1,random_num), random_num)
'''


with tf.variable_scope('LSTM_4_loc'):
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./LSTM_4_loc')
        ...
        with open("full_LSTM_loc_4_best.txt", "w") as f:
    ...
    mean_loss = np.mean(loss)
    print(np.max(loss), np.min(loss), mean_loss, np.std(loss))
    saver.save(sess, './LSTM_4_loc/LSTM_4_loc_test_result.ckpt')
