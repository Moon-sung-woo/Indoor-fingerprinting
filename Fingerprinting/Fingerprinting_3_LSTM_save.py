...
learning_rate = 0.008955754961947109
EPOCH = 5180
...
def test_data_random_sample(batch_size = 32, seq_length=1):
    ...
    test_y_data = []
    for i in range(batch_size):
        idx = random.randint(1, x_3_loc_test.shape[0]) - 1
        for j in range(seq_length):
...
with tf.variable_scope('LSTM_3_loc'):
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./LSTM_3_loc')
        ...
        with open("full_LSTM_loc_3_best.txt", "w") as f:
    ...
    #검증
        ...
        mean_loss = np.mean(loss)
        print(np.max(loss), np.min(loss), mean_loss, np.std(loss))
        saver.save(sess, './LSTM_3_loc/LSTM_3_loc_test_result.ckpt')
