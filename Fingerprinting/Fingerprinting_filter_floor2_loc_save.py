...
def test_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        ...
        test_x_data.extend(  x_2_loc_test[idx:idx+1]  )
...
with tf.variable_scope('filter_2'):
    # ---------------------------------------------------------------------------------------
    X = tf.placeholder(tf.float32, [None, x_2_loc_train.shape[1]])
    in_filter = tf.placeholder(tf.float32, [None, x_2_loc_train.shape[1]])
    ...
    layer1 = tf.contrib.layers.fully_connected(filter1, int(3598.561742740699), activation_fn=tf.nn.elu)
    ...
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(4401.782387938591), activation_fn=tf.nn.elu)
    ...
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(2092.7422025134206), activation_fn=tf.nn.elu)
...
with tf.Session() as sess:
    ...
    print('load model...')
    ckpt = tf.train.get_checkpoint_state('./filter_2')
    saver.restore(sess, ckpt.model_checkpoint_path)

    for i in range(992):
        print(i, "W = ", sess.run(W_filter)[i], "b = ", sess.run(b_filter)[i])
    ...
    if load_model:
        ...
        for i in range(1000):
            ...
            verification = x_2_loc_test.shape[0]
        ...
        with open("filter_full_loc_2_best.txt", "w") as f:
        ...
        mean_loss = np.mean(loss)
        print(np.max(loss), np.min(loss), mean_loss, np.std(loss))
        saver.save(sess, './filter_2/loc_floor2_test_result.ckpt')
    '''