...
floor_batch_size = 1
batch_size = 1000
learning_rate = 0.0028284061713242897
output_floor = 1
floor_size = int(floor_y_data.max()) + 1#5
loc_size = 2 #x,y
encoder_hidden_num1 = 350
encoder_hidden_num2 = 260
drop_out_param = 0.6817233929177008
EPOCH = 11680
load_model = True
random_num = int(x_0_loc_test.shape[0] * 0.1)
minimum_loss = 999999
minimum_snapshot = []
'''
#TUT 데이터 사용
...
with tf.variable_scope('filter_0'):
    ...
    in_filter = tf.placeholder(tf.float32, [None, x_0_loc_train.shape[1]])
    ...
    layer1 = tf.contrib.layers.fully_connected(filter1, int(2139.9871257184886), activation_fn=tf.nn.elu)
    ...
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(2618.5757009708777), activation_fn=tf.nn.elu)
    ...
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(4658.258478046493), activation_fn=tf.nn.elu)
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./filter_0')
        saver.restore(sess, ckpt.model_checkpoint_path)

        for i in range(1000):

            verification = x_0_loc_test.shape[0]
        ...
        with open("filter_full_loc_0_best.txt", "w") as f:
        ...
        mean_loss = np.mean(loss)
        print(np.max(loss), np.min(loss), mean_loss, np.std(loss))
        saver.save(sess, './filter_0/loc_floor0_test_result.ckpt')