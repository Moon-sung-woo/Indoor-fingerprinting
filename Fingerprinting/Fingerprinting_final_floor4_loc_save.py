...
floor_batch_size = 1
batch_size = 1000
learning_rate = 0.00824367334748697
output_floor = 1
floor_size = int(floor_y_data.max()) + 1#5
loc_size = 2 #x,y
encoder_hidden_num1 = 350
encoder_hidden_num2 = 260
drop_out_param = 0.580027926250951
EPOCH = 6183
load_model = True
random_num = int(x_4_loc_test.shape[0] * 0.5)
minimum_loss = 999999
minimum_snapshot = []
'''
#TUT 데이터 사용
...
def train_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        ...
        train_x_data.extend(x_4_loc_train[idx-seq_length+1: idx+1])
...
def test_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        ...
        test_x_data.extend(  x_4_loc_test[idx-seq_length+1 : idx+1]  )
...
print()
'''
with tf.variable_scope('final_4_loc'):
    # ---------------------------------------------------------------------------------------
    ...
    layer1 = tf.contrib.layers.fully_connected(X, int(1.56848007e+03), activation_fn=tf.nn.elu)
    ...
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(2.03228343e+03), activation_fn=tf.nn.elu)
    ...
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(2.45828014e+03), activation_fn=tf.nn.elu)
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./final_4_loc')
        ...
        with open("final_full_loc_4_best.txt", "w") as f:
    ...
    # 검증
        ...
        saver.save(sess, './final_4_loc/final_loc_floor4_test_result.ckpt')