...
W_filter = tf.Variable(tf.random_normal([input_dim, input_dim]))#520 -> 350
...
filter1 = tf.add(tf.matmul(X, W_filter), tf.multiply(in_filter, matrix_b))
...
with tf.Session() as sess:
    ...
    verification = x_0_loc_test
    filter_size = verification.shape[0] * seq_length

    test_x, test_y = test_data_sample(verification, y_0_loc_test, seq_length)
