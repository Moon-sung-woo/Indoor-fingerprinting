...
floor_batch_size = 1
batch_size = 1000
learning_rate = 1.79817274e-03
output_floor = 1
floor_size = int(floor_y_data.max()) + 1#5
loc_size = 2 #x,y
encoder_hidden_num1 = 350
encoder_hidden_num2 = 260
drop_out_param = 9.42149104e-01
EPOCH = 5000
load_model = True
random_num = int(x_test.shape[0] * 0.1)
minimum_loss = 999999
minimum_snapshot = []
'''
#TUT 데이터 사용
...
with tf.Session() as sess:
    ...
    if load_model:
        ...
        for i in range(1000):
            ...
            loss = sess.run([result_loss],
                            feed_dict={X: test_x, Y: test_y, in_batch_size: batch_size, in_drop_out: 1.0})
        ...
        with open("all_floor_best.txt", "w") as f:
    ...
    # 검증
        ...
        print( np.max(loss), np.min(loss), np.mean(loss), np.std(loss))
        saver.save(sess, './location/loc_test_result.ckpt')
