...
with tf.variable_scope('filter_1'):
    ...
    layer1 = tf.contrib.layers.fully_connected(filter1, int(3629.026027620676 ), activation_fn=tf.nn.elu)
    ...
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(1748.8292773909443), activation_fn=tf.nn.elu)
    ...
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(2219.743548257833), activation_fn=tf.nn.elu)
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./filter_1')
        ...
        with open("filter_full_loc_1_best.txt", "w") as f:
        ...
        mean_loss = np.mean(loss)
        print(np.max(loss), np.min(loss), mean_loss, np.std(loss))
        saver.save(sess, './filter_1/filter_loc_floor1_test_result.ckpt')