...
def train_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        ...
        train_x_data.extend(x_3_loc_train[idx-seq_length+1: idx+1])
...
def test_data_random_sample(batch_size = 32):
    ...
    for i in range(batch_size):
        ...
        test_x_data.extend(  x_3_loc_test[idx-seq_length+1 : idx+1]  )
...
print()
'''
with tf.variable_scope('final_3_loc'):
    # ---------------------------------------------------------------------------------------
    ...
    layer1 = tf.contrib.layers.fully_connected(X, int(2.18183588e+03), activation_fn=tf.nn.elu)
    ...
    layer2 = tf.contrib.layers.fully_connected(drop_1, int(3.20307837e+03), activation_fn=tf.nn.elu)
    ...
    layer3 = tf.contrib.layers.fully_connected(drop_2, int(2.27523949e+03), activation_fn=tf.nn.elu)
...
with tf.Session() as sess:
    ...
    if load_model:
        print('load model...')
        ckpt = tf.train.get_checkpoint_state('./final_3_loc')
        ...
        with open("final_full_loc_3_best.txt", "w") as f:
    ...
    # 검증
        ...
        saver.save(sess, './final_3_loc/final_loc_floor3_test_result.ckpt')